...
(function injectGoogleTranslate() {
  ...
  // Use https: explicitly — protocol-relative // breaks on file:// origins
...
window.googleTranslateElementInit = function () {
  ...
  // After the widget is ready, try to apply any saved language via the combo.
  // This is a belt-and-suspenders backup for the cookie approach.
  const saved = localStorage.getItem('necho_lang');
  if (saved && saved !== 'en') _triggerGTCombo(saved);
};

// Directly set Google Translate's combo select — retries while GT is loading.
function _triggerGTCombo(code) {
  ...
  const interval = setInterval(() => {
    if (apply() || ++tries >= 75) clearInterval(interval); // up to ~15 s
  }, 200);
}
...
function changeLanguage(code) {
  ...
  // Clear any previous translation cookie
  ...
  // Set the googtrans cookie so translation persists across reloads
  document.cookie = `googtrans=/en/${code}; path=/`;
  if (host) document.cookie = `googtrans=/en/${code}; path=/; domain=${host}`;

  // Try translating immediately; if the combo isn't ready yet, wait up to
  // 2 s for GT to initialise before falling back to a full page reload.
  const tryApply = () => {
    const combo = document.querySelector('.goog-te-combo');
    if (!combo) return false;
    combo.value = code;
  ...
  const interval = setInterval(() => {
    if (tryApply() || ++tries >= 10) {
      ...
      if (tries >= 10) location.reload(); // GT not ready — let cookie do it
    }
  }, 200);
}
