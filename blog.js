...
const DEFAULT_POSTS = [
  ...
  },
  {
    ...
    body: `
      ...
      <p>Current production systems work with closed vocabularies — sets of signs the model has been trained to recognize. This is workable for common scenarios but breaks down for proper nouns, technical terminology, and newly coined signs. The solution lies in combining gesture recognition with fingerspelling recognition (for spelling out words) and integrating large language model context to predict likely vocabulary from conversational context.</p>
      ...
      <p>Hands are only part of the picture. Non-manual markers — facial expressions, mouth movements, head tilt, shoulder positioning — carry grammatical information in ASL and most other sign languages. A raised eyebrow changes a statement into a yes/no question. Pursed lips indicate a specific degree of intensity. Current hand-centric models miss this entire layer of meaning.</p>
...
function seedDefaultPosts() {
  ...
  if (existing.length > 0) return; // don't overwrite user-created posts
